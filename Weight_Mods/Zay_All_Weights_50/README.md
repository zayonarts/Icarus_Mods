<div align="center">

# Zay - All Weights [-50%]

![Mod Version](https://img.shields.io/badge/Mod_Version-1.0-0078D4?style=for-the-badge)
![Icarus Week](https://img.shields.io/badge/Icarus_Week-251-2EA043?style=for-the-badge)
![Author](https://img.shields.io/badge/Author-Zayon-E36209?style=for-the-badge)
![Format](https://img.shields.io/badge/Format-.EXMODZ-8B5CF6?style=for-the-badge)

**A mod that reduces the weight of all items by 50%.**

</div>

---

## 📋 Mod Details

| Feature | Detail |
|---------|--------|
| **Items Modified** | 3385 items |
| **Weight Reduction** | -50% from original values |
| **Rounding** | Rounded down to 1 decimal place |
| **Data Table** | `D_Itemable` |
| **Mod Format** | `.EXMODZ` |
| **Multiplayer** | ✅ Single player, hosted MP & dedicated server |

> Each item's original `Weight` value is reduced by **50%** and rounded down
> to one decimal place. For example, an item weighing 10.0 becomes 5,
> and an item weighing 1.0 becomes 0.5.

## 📥 Installation

1. Download `Zay_All_Weights_50.EXMODZ`
2. Import (Zay_All_Weights_50.EXMODZ) into your Icarus Mod Manager.

> **Note:** This mod uses the `.EXMODZ` format and requires Icarus Mod Manager to install.

## ⚠️ Disclaimer

- Backup and save all user data before installing or using any mods
- **Use at your own risk**
- Ensure you are running the latest version compatible with your Icarus version

---

<div align="center">

**Made by Zayon**

</div>
